...
* 機能：開発者用のコンソール機能を追加 (CodeMeister)
  * bundle installを走らせる`bin/setup`を追加
  * `bin/console`を追加し、factory_botが読み込まれた状態でIRBコンソールを開けるようにしました
  * 全てのgemファイルを再読み込みするセッションメソッド`reload!`を追加
* 機能：before(:all) と after(:all) コールバックを追加 (CodeMeister)
* 機能：before buildコールバックの対応を追加（Mohammed Nasser氏とNeil Carvalho氏）
* 機能：遅延読込される初期値系列への対応を追加 (AJ Esler)
* 修正：`.irbrc`で、読み込まれた登録器からファイルを削除する処理を改修 (CodeMeister)
* 修正：系列の処理を改善し、よりよい文脈の対応がされるようにしました (CodeMeister)
  * イシュー #1754 を修正しました。
    `#generate`がエラーを投げる事象がありました
  * `#increment_sequence`メソッドを改修し、URIの要件を削除しました
* 修正：ActiveSupportコア拡張を整理 (Neil Carvalho)
* 修正：イシュー #1709 に対処しました。
  「<attribute>」と「<attribute_id>」の競合解決の部分です (CodeMeister)
* 修正：イシュー #1712 に対処しました。
  コールバックがインスタンス毎に1回だけ走るようにしました (CodeMeister)
* 文書：`.irbrc`ファイルの単語「constants」を小文字に (CodeMeister)
* 文書：文書を更新し、ファクトリで完全な名前空間が必要な場合を明記しました (John Pitchko)
* 微修正：新しいメンテナをCODEOWNERSに追加 (Ajina Slater)

## 6.5.4（2025年6月13日）

* ファクトリ内の`method_missing`を通じて`definition`という名前の利用者が定義したメソッドを設定できない不具合を修正しました。
  (CodeMeister)
...
* 文書：has_many-associations.mdを更新し、トレイトで行内関連を使えることに言及しました (Matthew Zagaja)
...
* @Earlopainにより、 https://github.com/thoughtbot/factory_bot/pull/1686 でrails
  7.2の互換性が確保されました
...
* 追加：`build_stubbed`でUUIDの主キーを検知したとき、正しい種別を生成するようにしました (Peter Boling,
  Alexandre Ruban)。
* 文書：Ruby 3の糖衣構文の例を示しました (Sean Doyle)。
* 内部的：テストの警告伝文を解決 (Mike Burns)。
...
* Fixed: ActiveSupport::Notifications reporting strategy through
  ...
  * BREAKING CHANGE: Custom strategies now need to define a `to_sym` method
    to specify the strategy identifier
...
* Changed: raise a more helpful error when passing an invalid argument to an
  association
...
* Added: automatic definition of traits for Active Record enum attributes,
  enabled by default (Note that this required changing where factory_bot
  constantizes the build class, which may affect applications that were
  using abstract factories for inheritance. See issue #1409.) (This may
  break `FactoryBot.lint` because there may be previously non-existing
  factory+trait combinations being defined and checked)
* Added: `traits_for_enum` method to define traits for non-Active Record
  enums
* Added: `build_stubbed_starting_id=` option to define the starting id for
  `build_stubbed`
* Removed: deprecated methods on the top-level `FactoryBot` module meant
  only for internal use
* Removed: support for EOL versions of Ruby (2.3, 2.4) and Rails (4.2)
* Removed: support for "abstract" factories with no associated class; use
  traits instead.
...
* Changed: use_parent_strategy now defaults to true, so by default the build
  strategy will build, rather than create associations
...
* Bugfix: avoid a stack error when referring to an "attributes" attribute in
  initialize_with
...
* Removed: duplicate_attribute_assignment_from_initialize_with configuration
  option
...
* Add `FactoryGirl.use_parent_strategy` configuration to allow associations
  to leverage parent build strategy
...
Update README to include info about running with JRuby * Update dependencies
on RSpec and tiny versions of Rails in Appraisal * Improve flexibility of
using traits with associations and add documentation * Stub update_column to
raise to mirror ActiveRecord's change from update_attribute
...
* Deprecate after_build, after_create, before_create, after_stub in favor of
  new callbacks
...
* Syntax methods that deal with ORM interaction (attributes_for, build,
  build_stubbed, and create) now accept a block that yields the result. This
  results in a more convenient way to interact with the result than using
  Object.tap.
* Standardize deprecation warnings
* Update transient attribute syntax to use blocks instead of calling ignore
  on each attribute declaration
...
* Declarations: allow overriding/modification of individual traits in child
  factories
